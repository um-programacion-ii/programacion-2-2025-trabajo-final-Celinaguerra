package com.celi.backend.service.catedra;
...
import com.celi.backend.service.catedra.exception.CatedraAuthenticationException;
import com.celi.backend.service.catedra.exception.MissingCatedraTokenException;
import com.celi.backend.service.dto.CatedraEventoDTO;
...
public class EventoSincronizacionService {
     ...
     * - Elimina eventos que ya no están en el servicio de cátedra (excepto los
     * cancelados)
    ...
    public void sincronizarEventos() {
        ...
        try {
            ...
            // Eliminar eventos que ya no están en el servicio de cátedra (excepto
            // cancelados)
    ...
    /**
     * Procesa un evento individual: lo crea si no existe o lo actualiza si ya
     * existe.
     */
    ...
    private void eliminarEventosNoExistentes(Set<Long> idsCatedra) {
        ...
        for (Evento evento : eventosLocales) {
            ...
            if (!idsCatedra.contains(evento.getEventoIdCatedra())) {
                LOG.debug("Eliminando evento local con ID cátedra: {} (ya no existe en servicio de cátedra)",
                        evento.getEventoIdCatedra());
     ...
     * Los eventos expirados se mantienen en la base de datos pero se filtran en las
     * consultas.
     */
