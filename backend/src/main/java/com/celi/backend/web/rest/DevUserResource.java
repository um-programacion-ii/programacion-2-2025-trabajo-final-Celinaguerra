 ...
 * for querying users without requiring ADMIN role, useful for development and
 * testing.
 */
@RestController
...
public class DevUserResource {
     ...
     * <p>
     * This endpoint is only available in development profile and does not require
     * ADMIN role.
     ...
     */
    @GetMapping("/users")
    public ResponseEntity<List<AdminUserDTO>> getAllUsersForDev(
        ...
        if (!env.acceptsProfiles(
                org.springframework.core.env.Profiles.of(JHipsterConstants.SPRING_PROFILE_DEVELOPMENT))) {
