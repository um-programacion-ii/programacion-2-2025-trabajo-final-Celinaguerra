...
public class SesionSeleccionResource {
    ...
    @PutMapping("/asientos")
    public ResponseEntity<Void> actualizarAsientosSeleccionados(
            @Valid @RequestBody EstadoSeleccionDTO.AsientoSeleccionadoDTO[] asientos) {
        String userId = SecurityUtils.getCurrentUserLogin().orElseThrow();
        LOG.debug("REST request para actualizar asientos seleccionados: userId={}, cantidad={}", userId,
                asientos.length);
    ...
    @PutMapping("/nombres")
    public ResponseEntity<Void> actualizarNombresPersonas(
            @RequestBody Map<String, EstadoSeleccionDTO.AsientoSeleccionadoDTO> nombresPorAsiento) {
        String userId = SecurityUtils.getCurrentUserLogin().orElseThrow();
