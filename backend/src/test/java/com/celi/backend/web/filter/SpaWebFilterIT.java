package com.celi.backend.web.filter;
...
class SpaWebFilterIT {
    ...
    void forwardUnmappedSecondLevelMapping() throws Exception {
        mockMvc.perform(get("/first-level/second-level")).andExpect(status().isOk())
                .andExpect(forwardedUrl("/index.html"));
    }

    @Test
    void forwardUnmappedThirdLevelMapping() throws Exception {
        mockMvc.perform(get("/first-level/second-level/third-level")).andExpect(status().isOk())
                .andExpect(forwardedUrl("/index.html"));
    }
    ...
    /**
     * This test verifies that any files that aren't permitted by Spring Security
     * will be forbidden.
     * If you want to change this to return isNotFound(), you need to add a request
     * mapping that
